...
${\Big)}$
Detta uttryck skulle bli extremt så jag introducerar en alias:
$k_i(\bar{x}+th) =$
...
**Största möjliga** $h_xh_yh_z$ $= c \cdot ||h||^3$ där c är en konstant.

Om man flyttar runt var $c$ och $n^3$ konstanterna så att dom räknas in i $K$ in är det enkelt att se att `(PROP 1)` är ekvivalent med det beviset efterfrågar:
