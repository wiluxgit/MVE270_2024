...
Då $y$ inte ändras längs $C_3$ är integationen längs denna lika med noll. ($dy$ är alltid exakt noll, inte bara godtyckligt nära).
$\int_C Qdx = \int_C Q(x,y)dx = {\int_C}_1 Q(x,y)dy + {\int_C}_2 Q(x,y)dy$
$= {\int_C}_1 Q(x_a,y)dy + {\int_C}_2 P(x_b,y)dy$

$C_1$ och $C_2$ kan parametriseras lika dant som för figuren utan raka linjer så uttrycket vi får
