...
Om man flyttar runt var $\frac{1}{6}$, $c$ och $n^3$ konstanterna så att dom räknas in i $K$ in är det enkelt att se att `(PROP 1)` är ekvivalent med det beviset efterfrågar:
...
> Antag att $\nabla \times {\bf{F}} = {\bf{0}}$ i ett enkelt sammangängande område $D$, Då är $\bf{F}$ konservativt, dvs det finns ett deriverbart gälte $\phi$ sådant att ${\bf{F}} = \nabla\phi$ i $D$
...
Använder Stoke theorem:
> RE:
> ${\bf S}$ är en yta
> $C$ är kantkurvan av ${\bf S}$ där $t$ går längs kurvan.
> $\int_{\bf S}(\nabla \times {\bf F}) \cdot d{\bf S} = \int_C {\bf F} \cdot d{\bf t}$

$\int_{\bf S}(\nabla \times {\bf F}) \cdot d{\bf S} = \int_C {\bf F} \cdot d{\bf t}$
$\int_{\bf S}(0) \cdot d{\bf S} = \int_C {\bf F} \cdot d{\bf t}$
$0 = \int_C {\bf F} \cdot d{\bf t}$
